...
def createLokiAddonProject(project_name, project_description, project_major_version, project_minor_version, project_patch_version):
    ...
    shutil.copytree(".\LokiAddonTemplate", base_project_dir)

    # rename template files to match new project name
    print("Renaming Visual Studio project template files")
    os.rename(os.path.join(base_project_dir, "LokiAddonTemplate.vcxproj"), os.path.join(base_project_dir, project_name + ".vcxproj"))
    os.rename(os.path.join(base_project_dir, "LokiAddonTemplate.vcxproj.filters"), os.path.join(base_project_dir, project_name + ".vcxproj.filters"))
    ...
    for line in fileinput.input(os.path.join(base_project_dir, project_name + ".vcxproj"), inplace=1):
        ...
        line = line.replace("$LokiAddonName$", project_name)
        print(line),
    ...
    os.rename(os.path.join(includes_dir, "LokiAddonTemplate.hpp"), os.path.join(includes_dir, project_name + ".hpp"))
    os.rename(os.path.join(source_dir, "LokiAddonTemplate.cpp"), os.path.join(source_dir, project_name + ".cpp"))
    ...
    for line in fileinput.input(os.path.join(source_dir, project_name + ".cpp"), inplace=1):
        line = line.replace("$LokiAddonName$", project_name)
        line = line.replace("$LokiAddonDescription$", project_description)
...
def main():
    parser = argparse.ArgumentParser(description='Creates a new Loki addon project from a template.')
    ...
    createLokiAddonProject(args.project_name, args.description, args.major_version, args.minor_version, args.patch_version)
